...
```
composer require rarksule/hashidable
```
...
$id = User::decode(3RwQaeoOR1E7qjYy); // 1
$hash = 3RwQaeoOR1E7qjYy::encode($id); // 3RwQaeoOR1E7qjYy

$query = User::whereHashid($hash); // using primary key
$instance = $query->get();

$query = User::whereHashid($hash,'age'); // using age Column
$instance = $query->get();
...
You can also extend the global configuration on a per-model basis. To do this, your model should implement the `rarksule\Hashidable\HashidableConfigInterface` and define the `hashidableConfig()` method on the model.
